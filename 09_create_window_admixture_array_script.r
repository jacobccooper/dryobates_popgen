	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/06_dryobates/07_admixture_windows"
	directory_name <- "13_dryo_admix"
	cluster <- "quanah"
	max_number_jobs <- 100
	
	# read in reference index
	# filtered to only include genotyped chromosomes
	ref_index <- read.table("dryobates_chromosomes.fai", stringsAsFactors=F)
	
	# minimum scaffold size to include
	min_scaffold_size <- 950000
	
	# define window size
	window_size <- 100000
	
	# subset fai file by minimum scaffold size
	ref_index <- ref_index[ref_index[,2] >= min_scaffold_size, ]
	...
	for(a in 1:nrow(ref_index)) {
		...
		for(b in 1:a_windows) {
			...
			admix_helper1[[counter]] <- a_chromosome
			admix_helper2[[counter]] <- a_start
			admix_helper3[[counter]] <- a_end
	...
	admix_helper1 <- unlist(admix_helper1)
	admix_helper2 <- unlist(admix_helper2)
	admix_helper3 <- unlist(admix_helper3)
	
	# calculate number of array jobs
	if(length(admix_helper3) > max_number_jobs) {
		n_jobs_per_array <- ceiling(length(admix_helper3) / max_number_jobs)
		n_array_jobs <- ceiling(length(admix_helper3) / n_jobs_per_array)
	} else {
		n_array_jobs <- length(admix_helper3)
		n_jobs_per_array <- 1
	}
	
	admix_helper1 <- c(admix_helper1, rep("x", n_jobs_per_array - length(admix_helper1) %% n_jobs_per_array))
	admix_helper2 <- c(admix_helper2, rep(1, n_jobs_per_array - length(admix_helper2) %% n_jobs_per_array))
	admix_helper3 <- c(admix_helper3, rep(1, n_jobs_per_array - length(admix_helper3) %% n_jobs_per_array))
	length(admix_helper3)
	write(admix_helper1, file=paste(directory_name, "/admix_helper_chrom.txt", sep=""), ncolumns=1)
	write(admix_helper2, file=paste(directory_name, "/admix_helper_start.txt", sep=""), ncolumns=1)
	write(admix_helper3, file=paste(directory_name, "/admix_helper_end.txt", sep=""), ncolumns=1)

	# write the array script
	a.script <- paste(directory_name, "/admix100kbp_array.sh", sep="")
	...
	write("\tchrom_array=$( head -n${run} admix_helper_chrom.txt | tail -n1 )", file=a.script, append=T)
	write("", file=a.script, append=T)
	write("\tstart_array=$( head -n${run} admix_helper_start.txt | tail -n1 )", file=a.script, append=T)
	write("", file=a.script, append=T)
	write("\tend_array=$( head -n${run} admix_helper_end.txt | tail -n1 )", file=a.script, append=T)
	write("", file=a.script, append=T)
	
	# add header to output file
	header <- paste('\tgunzip -cd ', project_directory, '/CM026036.1.recode.vcf.gz | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
	...
	write("", file=a.script, append=T)
	# run admixture
	admixture_commands <- paste0("\tfor K in 3; do admixture -C 0.000001 --cv ",project_directory, "/windows/${chrom_array}__${start_array}__${end_array}__b.ped $K; done")
	write(admixture_commands, file=a.script, append=T)
	write("", file=a.script, append=T)
